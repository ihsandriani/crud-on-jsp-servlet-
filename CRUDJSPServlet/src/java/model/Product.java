...
public class Product {
        ...
        public ResultSet readByID() {
            try {
                Connection con = kon.openConnection();
                String sql = "select * from tb_produk where idprod = ?";
                PreparedStatement ps = con.prepareStatement(sql);
                ps.setString(1, this.IDProd);
        ...
        public int update() {
            try {
                Connection con = kon.openConnection();
                String sql = "update tb_produk set nama=?, stok=?, price=? where idprod=?";
                PreparedStatement ps = con.prepareStatement(sql);
        ...
        public int destroy() {
            try {
                Connection con = kon.openConnection();
                String sql = "delete from tb_produk where idprod = ?";
                PreparedStatement ps = con.prepareStatement(sql);
                ps.setString(1, this.IDProd);
                int res = ps.executeUpdate();
                return res;
